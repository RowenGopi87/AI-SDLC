          ...
          {selectedWorkItem ? (
            filteredTestCases.length > 0 ? (
              <Table>
                                 <TableHeader>
                   <TableRow>
                     <TableHead className="w-16 text-xs p-2">ID</TableHead>
                     <TableHead className="min-w-0 max-w-60 text-xs p-2">Title</TableHead>
                     <TableHead className="w-12 text-xs p-2">Type</TableHead>
                     <TableHead className="w-16 text-xs p-2">Status</TableHead>
                     <TableHead className="w-16 text-xs p-2">Priority</TableHead>
                     <TableHead className="w-20 text-xs p-2">Assignee</TableHead>
                     <TableHead className="w-32 text-xs p-2">Actions</TableHead>
                   </TableRow>
                 </TableHeader>
                <TableBody>
                  {filteredTestCases.map((testCase) => (
                    ...
                    >
                       ...
                       <TableCell className="min-w-0 max-w-60 p-2">
                         <div className="min-w-0">
                           <div className="font-medium text-xs truncate" title={testCase.title}>
                             {testCase.title.length > 35 ? `${testCase.title.substring(0, 35)}...` : testCase.title}
                           </div>
                           <div className="text-xs text-gray-600 truncate" title={testCase.summary}>
                             {testCase.summary.length > 40 ? `${testCase.summary.substring(0, 40)}...` : testCase.summary}
                           </div>
                         </div>
                       </TableCell>
                      ...
                      <TableCell className="p-2">
                        <Badge className={`${getStatusColor(testCase.status)} text-xs h-5 px-1`} variant="secondary">
                          {testCase.status.replace('_', ' ').toUpperCase()}
                        </Badge>
                      </TableCell>
                      <TableCell className="p-2">
                        <Badge className={`${getPriorityColor(testCase.priority)} text-xs h-5 px-1`} variant="outline">
                          {testCase.priority.toUpperCase()}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-xs p-2">{testCase.assignee || 'Unassigned'}</TableCell>
